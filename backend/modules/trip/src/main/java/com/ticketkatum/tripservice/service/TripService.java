...
public class TripService {
    ...
    public JourneyDTO getJourney(Long journeyId) {
        log.debug("Fetching journey by ID: {}", journeyId);
        Journey journey = journeyRepository.findById(journeyId)
                .orElseThrow(() -> new RuntimeException("Journey not found with ID: " + journeyId));
        return JourneyDTO.fromEntity(journey);
    }
    ...
    @Transactional(readOnly = true)
    public JourneyDTO getJourneyByTrip(Long tripId) {
        log.debug("Fetching journey for trip: {}", tripId);
        Trip trip = tripRepository.findById(tripId)
             .orElseThrow(() -> new RuntimeException("Trip not found with ID: " + tripId));
        
        if (trip.getJourneys() == null || trip.getJourneys().isEmpty()) {
            return null;
        }
        // Assuming single journey for now as per initializeJourney logic
        return JourneyDTO.fromEntity(trip.getJourneys().get(0));
    }
