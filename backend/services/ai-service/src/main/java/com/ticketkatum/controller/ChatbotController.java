...
import com.ticketkatum.service.ChatResponse;
...
public class ChatbotController {
        ...
        @PostMapping
        public ResponseEntity<Response<ChatResponse>> chat(
                        @RequestHeader("X-User-Id") String userId,
                        @RequestBody ChatRequest request) {
                log.info("Chat request from user: {} - message: {}", userId, request.message());

                ChatResponse chatResponse = chatbotService.chat(userId, request.message());
