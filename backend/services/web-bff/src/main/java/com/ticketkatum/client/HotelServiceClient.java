...
public class HotelServiceClient {
        ...
        public CompletableFuture<HotelDTO> getHotelById(Long hotelId) {
                ...
                return getWebClient()
                                ...
                                .onStatus(HttpStatus.NOT_FOUND::equals,
                                                response -> Mono.error(
                                                                new RuntimeException("Hotel not found: " + hotelId)))
        ...
        public CompletableFuture<List<com.ticketkatum.dto.hotel.booking.RentTypeDto>> getRentTypes() {
                log.debug("Fetching rent types");

                return getWebClient()
                                .get()
                                .uri("/api/v1/hotels/rent-types")
                                .retrieve()
                                .bodyToMono(Response.class)
                                .map(response -> objectMapperList(response.getData(),
                                                com.ticketkatum.dto.hotel.booking.RentTypeDto.class))
                                .toFuture();
        }

        public CompletableFuture<List<com.ticketkatum.dto.hotel.booking.MealPlanDto>> getMealPlans() {
                log.debug("Fetching meal plans");

                return getWebClient()
                                .get()
                                .uri("/api/v1/hotels/meal-plans")
                                .retrieve()
                                .bodyToMono(Response.class)
                                .map(response -> objectMapperList(response.getData(),
                                                com.ticketkatum.dto.hotel.booking.MealPlanDto.class))
                                .toFuture();
        }
}