...
import { Bus, Hotel, Plane, Calendar, ArrowRightLeft, Search } from 'lucide-react';
...
export function Homepage() {
  ...
  const [activeTab, setActiveTab] = useState('bus'); // bus, hotel, flight
...
// Popular Route Card
const PopularRoute = ({ from, to, price, time, img }) => (
  <div className="group relative overflow-hidden rounded-2xl aspect-video cursor-pointer shadow-xl hover:shadow-2xl transition-all">
    <img src={img} alt={`${from} to ${to}`} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
    <div className="absolute inset-0 bg-gradient-to-t from-black via-black/60 to-transparent p-6 flex flex-col justify-end">
      <div className="text-sm text-white/70 mb-2">{time}</div>
      <h3 className="text-2xl font-bold text-white mb-2">{from} → {to}</h3>
      <div className="flex justify-between items-center">
        <span className="text-orange-400 font-semibold">From NPR {price}</span>
        <button className="px-4 py-2 bg-orange-500 text-white rounded-lg text-sm font-semibold opacity-0 group-hover:opacity-100 transition-opacity">Book</button>
      </div>
    </div>
  </div>
);
...
// Testimonial Card
const Testimonial = ({ name, role, rating, text, avatar }) => (
  <div className="p-6 bg-gray-800/50 backdrop-blur-sm rounded-2xl border border-gray-700/50 hover:border-orange-500/50 transition-all">
    <div className="flex items-center gap-4 mb-4">
      <img src={avatar} alt={name} className="w-12 h-12 rounded-full border-2 border-orange-500" />
      <div>
