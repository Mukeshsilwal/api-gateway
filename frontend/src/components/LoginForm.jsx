...
export default function LoginForm({ onSuccess, redirectOnSuccess = true, transparent = false, className = "" }) {
    ...
    async function handleLogin() {
        ...
        try {
            ...
            let errorMessage = errorInfo.message || "Invalid email or password";
            const suggestion = getRecoverySuggestion(error);

            // Track Failure
            analytics.trackEvent('login_failure', { error: errorMessage });

            if (suggestion && errorInfo.type !== ErrorType.VALIDATION) {
    ...
    const transparentLabelStyles = transparent ? "text-indigo-100" : "";
