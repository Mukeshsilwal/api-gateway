...
interface AddHotelModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSave: (hotelData: HotelWizardState) => void;
}

export interface WizardRoom {
    id: number;
    roomNumber: string;
    roomType: string; // Mapped from 'type'
    basePrice: number;
    maxPrice?: number;
    capacity: number;
    description: string;
...
export interface HotelWizardState {
    name: string;
    hotelCode: string;
    address: string;
    city: string;
...
interface CurrentRoomState {
    ...
    amenities: string; // Comma separated string for input
    active: boolean;
}

export const AddHotelModal: React.FC<AddHotelModalProps> = ({ isOpen, onClose, onSave }) => {
    const [step, setStep] = useState(1);
    const [hotelData, setHotelData] = useState<HotelWizardState>({
        name: '',
        hotelCode: '',
        address: '',
        city: '',
    ...
    const handleHotelChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        setHotelData(prev => ({
            ...prev,
            [name]: value,
            ...(name === 'rating' ? { stars: parseInt(value) || 5 } : {})
        }));
    };
    ...
    const addRoom = () => {
        ...
        const basePrice = typeof currentRoom.basePrice === 'string' ? parseFloat(currentRoom.basePrice) : currentRoom.basePrice;
        ...
        if (currentRoom.maxPrice) {
            const parsedMax = typeof currentRoom.maxPrice === 'string' ? parseFloat(currentRoom.maxPrice) : currentRoom.maxPrice;
            if (!isNaN(parsedMax)) {
                maxPrice = parsedMax;
            }
        }

        if (maxPrice && (maxPrice <= 0 || maxPrice > 999999.99)) {
            alert('Max price must be between 0.01 and 999,999.99');
            return;
        }

        const capacity = typeof currentRoom.capacity === 'string' ? parseInt(currentRoom.capacity) : currentRoom.capacity;
                                        ...
                                        <div className="space-y-2">
                                            {hotelData.rooms.map((room) => (
                                                <div key={room.id} className="flex justify-between items-center p-3 bg-white border border-gray-200 rounded-lg shadow-sm">
                                                    <div>
                                                        <p className="font-medium text-gray-900">Room {room.roomNumber} - {room.roomType}</p>
                                            ...
                                            <tbody className="bg-white divide-y divide-gray-200">
                                                {hotelData.rooms.map((room) => (
                                                    <tr key={room.id}>
                                                        ...
                                                        <td className="px-4 py-2 text-sm text-gray-900">{room.roomType}</td>
