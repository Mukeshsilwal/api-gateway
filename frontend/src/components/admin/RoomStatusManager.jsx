...
export function RoomStatusManager({ isOpen, onClose, room, hotel, onUpdate }) {
    ...
    useEffect(() => {
        if (room && isOpen) {
            setRoomData({
                ...
                cleaningStatus: room.cleaningStatus || 'Pending',
                amenitiesStatus: room.amenitiesStatus || {
                    ac: true,
                    tv: true,
                    wifi: true,
                    hot_water: true,
    ...
    const fetchStaffList = async (hotelId) => {
        try {
            ...
            // Don't toast here to avoid spamming if multiple things fail, or just silent fail
        } finally {
    ...
    const loadMaintenanceRecord = async (roomId) => {
        try {
            ...
            if (record) {
                ...
                setRoomData(prev => ({
                    ...prev,
                    roomStatus: record.roomStatus || prev.roomStatus,
            ...
            // Don't show error toast for 404 - it's normal if no record exists yet
        } finally {
    ...
    const getSuggestions = () => {
        ...
        if (roomData.roomStatus === 'Occupied' && roomData.cleaningStatus !== 'Pending') {
            suggestions.push('Schedule cleaning after checkout.');
        }
        if (roomData.roomStatus === 'Available' && roomData.cleaningStatus !== 'Completed') {
    ...
    const handleAmenityToggle = (key) => {
        ...
        // We use roomData here which is the current state before update
        ...
        setRoomData(prev => {
            ...
            let newRoomStatus = prev.roomStatus;
    ...
    const handleSave = async () => {
        try {
            ...
            const maintenanceData = {
                ...
                maintenanceStatus: roomData.maintenanceStatus,
                            ...
                            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                                    {STATUS_OPTIONS.map(option => (
                                        <button
                                            key={option.value}
                                            onClick={() => setRoomData({ ...roomData, roomStatus: option.value })}
                                            className={`p-4 rounded-xl border-2 text-left transition-all ${roomData.roomStatus === option.value
                                                ? `border-indigo-500 ring-1 ring-indigo-500 ${option.color.replace('text-', 'bg-').replace('bg-', 'bg-opacity-10 ')}`
                                                : 'border-gray-200 hover:border-gray-300 bg-white'
                                                }`}
                                        >
                                            <div className={`font-semibold ${roomData.roomStatus === option.value ? 'text-indigo-700' : 'text-gray-700'}`}>
                                                {option.label}
                                            </div>
                                        </button>
                                    ))}
