...
interface AddRoomModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSave: (roomData: any) => Promise<void>; // Ideally specific DTO for creation but 'any' allows flexibility for now
    editingRoom?: AvailableRoomDto | null;
    existingRooms?: AvailableRoomDto[];
}
...
export const AddRoomModal: React.FC<AddRoomModalProps> = ({ isOpen, onClose, onSave, editingRoom = null, existingRooms = [] }) => {
    ...
    useEffect(() => {
        if (editingRoom) {
            const room = editingRoom as any; // Safe cast to access potential extra fields
            setFormData({
                ...
                pricingConfigurations: room.pricingConfigurations || [],
                allowedRentTypes: room.allowedRentTypes || ['DAILY'],
                allowedMealPlans: room.allowedMealPlans || ['NONE'],
                allowedMealServices: room.allowedMealServices || ['BUFFET'],
                images: room.images || []
            });
        } else {
            setFormData({
    ...
    const validate = (): boolean => {
        ...
        const basePrice = parseFloat(formData.basePrice as string);
        ...
        if (formData.maxPrice) {
            const maxPrice = parseFloat(formData.maxPrice as string);
        ...
        if (formData.pricingConfigurations.length > 0) {
            const invalidConfig = formData.pricingConfigurations.find(config =>
                !config.price || isNaN(parseFloat(config.price as string)) || parseFloat(config.price as string) < 0
            );
            if (invalidConfig) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const payload = {
                ...
                maxPrice: formData.maxPrice ? parseFloat(formData.maxPrice as string) : null,
                capacity: typeof formData.capacity === 'string' ? parseInt(formData.capacity) : formData.capacity,
                amenities: formData.amenities,
                images: uploadedImageUrls,
                active: formData.active,
                description: formData.description,
                pricingConfigurations: formData.pricingConfigurations.map(c => ({
                    ...c,
    ...
    const handlePricingConfigChange = (index: number, field: keyof PricingConfiguration, value: string | number) => {
        const newConfigs = [...formData.pricingConfigurations];
        newConfigs[index] = { ...newConfigs[index], [field]: value };
        setFormData(prev => ({ ...prev, pricingConfigurations: newConfigs }));
    };

    const handlePricingConfigRemove = (index: number) => {
        setFormData(prev => ({
            ...prev,
    ...
    const handleMultiSelectChange = (field: keyof RoomFormData, value: string) => {
        setFormData(prev => {
    ...
    const activeRooms = existingRooms.filter((r: any) => r.active !== false);
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex">
                ...
                {!editingRoom && activeRooms.length > 0 && (
                    <div className="w-80 bg-gradient-to-br from-gray-50 to-gray-100 border-r border-gray-200 p-5 overflow-y-auto hidden md:block">
