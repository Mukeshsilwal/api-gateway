 ...
 * - FAB for quick add
 */
const EnhancedRoomMaintenancePage = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            for (const hotel of hotelsData) {
                try {
                    const hotelRooms = await hotelService.getRoomsByHotel(hotel.hotelCode || hotel.id);
                    const roomsData = hotelRooms.data || hotelRooms || [];
    ...
    const filteredRooms = rooms.filter(room => {
        if (filters.hotelId && room.hotelId !== filters.hotelId) return false;
