...
import { Search, Calendar, Bell, ChevronDown, LayoutGrid, Bed, Film, Bus, Ticket as TicketIcon, Clock, AlertTriangle, Users, LucideIcon } from 'lucide-react';
...
const TICKETING_STATS: Record<string, Stats> = {
...
const TicketingDashboard: React.FC = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Process Bus Data
            const busData = (busRes as any)?.data || busRes || [];
            const busList = Array.isArray(busData) ? busData : (busData.bookings || busData.data || []);

            const mappedBusTickets: Ticket[] = busList.map((booking: any) => ({
                id: `BUS-${booking.id}`,
            ...
            // Process Hotel Data
            const hotelData = (hotelRes as any)?.data || hotelRes || [];
            const hotelList = Array.isArray(hotelData) ? hotelData : (hotelData.bookings || hotelData.data || []);

            const mappedHotelTickets: Ticket[] = hotelList.map((booking: any) => ({
            ...
            // Combine Real Data
            let allTickets: Ticket[] = [...mappedBusTickets, ...mappedHotelTickets];
    ...
    const mapStatus = (status: string | undefined): Ticket['status'] => {
    ...
    const calculatePriority = (date: string | undefined): Ticket['priority'] => {
        if (!date) return 'low';
    ...
    const calculateTimeLeft = (date: string | undefined): string => {
        if (!date) return '-';
        const travelDate = new Date(date);
        const now = new Date();
