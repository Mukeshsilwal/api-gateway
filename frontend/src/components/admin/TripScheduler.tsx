...
const TripScheduler: React.FC = () => {
    ...
    const [viewMode] = useState<'day' | 'week'>('week');
    const [draggedTrip, setDraggedTrip] = useState<MockUnassignedTrip | null>(null);

    // Mock Data
    const [buses] = useState<MockBus[]>([
    ...
    const [unassignedTrips, setUnassignedTrips] = useState<MockUnassignedTrip[]>([
    ...
    const [selectedTrip, setSelectedTrip] = useState<MockUnassignedTrip | null>(null);

    const handleTripSelect = (trip: MockUnassignedTrip) => {
    ...
    const handleSlotClick = (busId: number, date: Date) => {
    ...
    const handleDragStart = (e: React.DragEvent, trip: MockUnassignedTrip) => {
        setDraggedTrip(trip);
        e.dataTransfer.setData('tripId', String(trip.id));
    ...
    const handleDrop = (e: React.DragEvent, busId: number, date: Date) => {
        e.preventDefault();
