...
public class MainBffController {
    ...
    @Operation(summary = "Get home page data",
            description = "Returns all data needed for home page rendering")
    public CompletableFuture<ResponseEntity<Response<CompleteHomePageData>>> getCompleteHomePageData(
        ...
        CompletableFuture<HomePageData> hotelDataFuture =
                hotelAggregator.getHomePageData(userId, lat, lon);
...
//                            .onlineUserCount(onlineCountFuture.join())
    ...
    @Operation(summary = "Get complete user profile",
            description = "Returns user profile with all related data")
    public CompletableFuture<ResponseEntity<Response<CompleteUserProfile>>> getCompleteUserProfile(
        ...
        CompletableFuture<AggregatedUserDashboard> dashboardFuture =
                authAggregator.getUserDashboard(username, userId);

        CompletableFuture<BookingHistoryResponse> bookingHistoryFuture =
                bookingAggregator.getBookingHistory(userId, 0, 10);
     ...
     */
    @GetMapping("/health")
    @Operation(summary = "BFF Health Check",
            description = "Check health of Web-BFF and downstream services")
